...
# Upload a media file
@router.post("/media/")
async def upload_typescript_file(file: UploadFile = File(...), current_user: User = Depends(get_current_user)):
    """
...
@router.get('/images/')
async def list_typescript_images(current_user: User = Depends(get_current_user)):
    """
...
async def delete_typescript_image(filename: str, current_user: User = Depends(get_current_user)):
    """
